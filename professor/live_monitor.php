...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 1. GANTI FASE MANUAL (Klik Timeline)
    if (isset($_POST['set_phase'])) {
        $new_phase = $_POST['set_phase'];
        
        $col_a = strtolower($new_phase) . '_a';
        $col_b = strtolower($new_phase) . '_b';
        
        // Ambil data lama dari history kolom agar tidak hilang saat switch manual
        $refresh = query("SELECT $col_a, $col_b FROM missions WHERE id=$id_misi")[0];
        $old_a = $refresh[$col_a];
        $old_b = $refresh[$col_b];

        mysqli_query($conn, "UPDATE missions SET 
    ...
    // 2. KIRIM PERINTAH & AUTO-NEXT PHASE
    if (isset($_POST['update_monitor'])) {
        $current_phase = $misi_data['current_phase']; 
        $fase_aktif_db = strtolower($current_phase); 
        
        $opsi_a = htmlspecialchars($_POST['opsi_a_text']);
        $opsi_b = htmlspecialchars($_POST['opsi_b_text']);
        
        // A. SIMPAN KE ARSIP KOLOM SPESIFIK
        $col_a = $fase_aktif_db . '_a';
        $col_b = $fase_aktif_db . '_b';
        mysqli_query($conn, "UPDATE missions SET $col_a = '$opsi_a', $col_b = '$opsi_b' WHERE id=$id_misi");

        // B. TENTUKAN FASE BERIKUTNYA
        $next_phase = $current_phase;
        if ($current_phase == 'Planning') {
            $next_phase = 'Execution';
        } elseif ($current_phase == 'Execution') {
            $next_phase = 'Negotiation';
        } elseif ($current_phase == 'Negotiation') {
            $next_phase = 'Escape';
        } 
        // Escape tetap Escape
        ...
        if ($next_phase !== $current_phase) {
            // -- LOGIKA PINDAH FASE (RESET TEXT) --
            
            // Ambil data fase tujuan (biasanya kosong)
            $next_col_a = strtolower($next_phase) . '_a';
            $next_col_b = strtolower($next_phase) . '_b';
            
            $next_data = query("SELECT $next_col_a, $next_col_b FROM missions WHERE id=$id_misi")[0];
            $display_a = $next_data[$next_col_a]; 
            $display_b = $next_data[$next_col_b]; 
            
            mysqli_query($conn, "UPDATE missions SET 
                                 current_phase='$next_phase', 
                                 option_a_text='$display_a', 
                                 option_b_text='$display_b' 
                                 WHERE id=$id_misi");
        } else {
            // -- LOGIKA TETAP DI FASE SAMA (KHUSUS ESCAPE) --
            
            mysqli_query($conn, "UPDATE missions SET 
                                 option_a_text='$opsi_a', 
                                 option_b_text='$opsi_b' 
                                 WHERE id=$id_misi");
                                 
            // [BARU] ALERT KHUSUS FASE TERAKHIR
            if ($current_phase == 'Escape') {
                echo "<script>alert('INSTRUKSI BERHASIL DIKIRIM! JALANKAN MISI!');</script>";
            }
        }
    }
    ...
    if (isset($_POST['hapus_instruksi'])) {
        $fase_aktif = strtolower($misi_data['current_phase']);
        $reset = "";
        
        $col_a = $fase_aktif . '_a';
        $col_b = $fase_aktif . '_b';
        
        mysqli_query($conn, "UPDATE missions SET $col_a='$reset', $col_b='$reset', option_a_text='$reset', option_b_text='$reset' WHERE id=$id_misi");
    }
...
<head>
    ...
    <style>
        .instr-input-box {
            ...
            border: 2px solid #C31C22 !important;
        ...
        <div class="mission-sidebar">
            ...
            <div class="sidebar-menu">
                <div class="menu-item active" style="margin-top: 10px;">
                    <span class="monitor-icon">🖥</span> LIVE MONITOR
                </div>
            </div>
            ...
            <div class="monitor-split">
                <div class="phase-card">
                    <div class="phase-content">
                        ...
                        <p id="phaseDesc" class="phase-desc">STATUS: ACTIVE<br>EDIT & SIMPAN PERINTAH UNTUK FASE INI</p>
                    </div>
                </div>

                <div class="instruction-panel">
                    <h2 class="instr-title">INSTRUKSI PROFESSOR (<?= strtoupper($misi_data['current_phase']); ?>)</h2>
                    ...
                    <div class="instr-actions-row">
                        <button type="submit" name="hapus_instruksi" class="action-small-btn delete" onclick="return confirm('Hapus Instruksi?')">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="square" stroke-linejoin="miter"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg> DELETE
                        </button>
                    </div>
